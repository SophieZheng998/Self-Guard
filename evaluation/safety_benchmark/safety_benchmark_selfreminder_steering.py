...
def prepare_model_and_tokenizer(model_name, mem_percentage):
    model = LLM(model_name, gpu_memory_utilization=mem_percentage)
...
def prepare_dataset(data_path):
    ...
    if data_path == "walledai/AdvBench":
        ...
        for example in tqdm(dataset):
            ...
            formatted_dataset.append(data)
    elif data_path == "gcg_advbench_qwen3_8b":
...
class Evaluator:
    BATCH_SIZE = 4096
    def __init__(self, num_gpus=1, model='Llama-Guard', mem_percentage=0.95):
        if model == 'Llama-Guard':
            self.evaluator = 'Llama-Guard'                                                                                                            
            # model_id = "meta-llama/Llama-Guard-3-8B"
            model_id = "Tina-xxxx/Llama-Guard-3-8B"
            self.model = LLM(model=model_id, gpu_memory_utilization=mem_percentage, tensor_parallel_size=num_gpus, max_num_seqs=128)
...
def main(args):
    ...
    model, tokenizer = prepare_model_and_tokenizer(model_name, mem_percentage)
    ...
    if eval_model_name == "Llama-Guard" or eval_model_name == "ft-mistral-7b-instruct-v0.2":
        evaluator = Evaluator(model=eval_model_name, mem_percentage=mem_percentage)
