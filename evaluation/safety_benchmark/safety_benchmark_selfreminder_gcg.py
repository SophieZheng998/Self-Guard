...
def prepare_model_and_tokenizer(model_name):
    model = LLM(model_name, gpu_memory_utilization=0.65, enforce_eager=True)
...
class Evaluator:
    ...
    def __init__(self, num_gpus=1, model='Llama-Guard'):
        if model == 'Llama-Guard':
            ...
            self.model = LLM(model=model_id, gpu_memory_utilization=0.65, tensor_parallel_size=num_gpus, max_num_seqs=128, enforce_eager=True)
