...
def batch_calculate_attention(
    ...
    """
    批量计算多个文本的attention matrix
    
    参数:
        texts: 文本列表，每个元素是一句话
        model: 预加载的模型
        tokenizer: 对应的tokenizer
        padding: padding策略 ("longest" 或 "max_length")
    
    返回:
        results: 包含attention matrices和相关信息的字典
            - attentions: list of tuple，每个元素对应一个句子的所有层attention
            - input_ids: batch的input_ids
            - attention_mask: batch的attention_mask
            - tokens: 每个句子的token列表
            - seq_lengths: 每个句子的实际长度（不包括padding）
    """
    ...
    inputs = tokenizer(
        texts,
        return_tensors="pt",
        # padding=padding,  # 自动padding到最长序列
        padding="max_length",
        truncation=True,
        max_length=200,
    ...
    # print(f"Padded sequence length: {inputs['input_ids'].shape[1]}")
    
    # 获取每个句子的实际长度（不包括padding）
    seq_lengths = inputs['attention_mask'].sum(dim=1).tolist()
    
    # 批量计算attention
    with torch.no_grad():
        outputs = model(**inputs, output_attentions=True)
    
    # outputs.attentions是一个tuple，每个元素是一层的attention
    # 每层的attention shape: (batch_size, num_heads, seq_len, seq_len)
    batch_attentions = outputs.attentions
    
    # 将batch attention分解为每个句子的attention
    individual_attentions = []
    tokens_list = []
    
    for i in range(len(texts)):
        # 提取第i个句子的attention（所有层）
        sentence_attentions = []
        for layer_attn in batch_attentions:
            # layer_attn shape: (batch, heads, seq_len, seq_len)
            # 提取第i个样本，实际长度部分
            actual_len = seq_lengths[i]
            sentence_layer_attn = layer_attn[i:i+1, :, :actual_len, :actual_len]
            sentence_attentions.append(sentence_layer_attn)
        
        individual_attentions.append(tuple(sentence_attentions))
        
        # 获取tokens（去除padding）
        tokens = tokenizer.convert_ids_to_tokens(
            inputs['input_ids'][i][:seq_lengths[i]]
        )
        tokens_list.append(tokens)
    
    results = {
        'attentions': individual_attentions,  # list of tuples
...
def calculate_IAS(
    attentions: tuple, # (4, 32, 1, 32, 7, 7)
    t_inj: int,
    L: int,
    B: int,
    ...
    """
    计算每个reasoning path的Injection Attention Score (IAS)
    
    参数:
        attentions: tuple of attention tensors from model
                   每个tensor形状: (batch_size, num_heads, seq_len, seq_len)
                   或者是list of tuples (对应多个paths)
        t_inj: 注入safety aha phrase的位置
        L: 注入短语的长度
        B: budget window的步数
        path_lengths: 每个path的总长度列表 [T_p1, T_p2, ...]
    
    返回:
        ias_scores: 每个path的IAS分数列表
    """
    
    num_paths = len(attentions)
    ias_scores = []
    
    # 对每个path计算IAS
    for path_idx in range(num_paths):
        path_attentions = attentions[path_idx] # (32, 1, 32, 7, 7)
        T_p = path_lengths[path_idx]
        
        # 计算budget window的起始位置 t_0
        t_0 = t_inj + L
        
        # 确保budget window不超过path长度
        actual_B = min(B, T_p - t_0)
        ...
        # 获取attention维度信息
        first_layer = path_attentions[0]
        batch_size, num_heads_per_layer, seq_len, _ = first_layer.shape
        H = num_heads_per_layer * len(path_attentions)  # 总head数
        
        total_score = 0.0
        
        # 对budget window中的每个timestep t ∈ [t_0, t_0 + B - 1]
        for t in range(t_0, t_0 + actual_B):
            # 计算temporal weighting factor γ_t = (t+1) / B
            gamma_t = (t - t_0 + 1) / actual_B
            
            timestep_score = 0.0
            
            # 遍历所有层
            for layer_idx, layer_attn in enumerate(path_attentions): # (32, 1, 32, 7, 7)
                # 遍历该层的所有heads
                for h in range(num_heads_per_layer):
                    # 计算从位置t的query token到整个注入短语的attention
                    # 注入短语范围: [t_inj, t_inj + L - 1]
                    
                    # A^l_h[t, k] 表示位置t对位置k的attention
                    attention_to_injection = layer_attn[0, h, t, t_inj:t_inj+L].sum().item()
                    
                    timestep_score += attention_to_injection
            
            # 对所有heads取平均
            timestep_score /= H
            
            # 应用temporal weighting
            total_score += gamma_t * timestep_score
        
        # 对budget window取平均
        ias = total_score / actual_B
        ias_scores.append(ias)
    
    return ias_scores

def select_optimal_path(ias_scores: List[float]) -> Tuple[int, float]:
    """
    根据IAS分数选择最优的reasoning path
    
    参数:
        ias_scores: 每个path的IAS分数
    
    返回:
        optimal_idx: 最优path的索引
        optimal_score: 对应的IAS分数
    """
    optimal_idx = np.argmax(ias_scores)
    optimal_score = ias_scores[optimal_idx]
    
    return optimal_idx, optimal_score
...
def main(args):
    ...
    L = len(tokenizer.tokenize(insert_instruction))
    B = 25
    ...
    for data in tqdm(dataset):
        ...
        pre_response = data["new_prompt"][len(data["prompt"]):]
        t_inj = len(tokenizer.tokenize(pre_response)) - L
        total_response_list = []
        for response in data["response"]:
            total_response_list.append(pre_response + " " + response)
        
        results = batch_calculate_attention(total_response_list, model, tokenizer)
        ias_scores = calculate_IAS(
            attentions=results['attentions'],
            t_inj=t_inj,
            L=L,
            B=B,
            path_lengths=results['seq_lengths']
        )
        # print(ias_scores)
        optimal_idx, optimal_score = select_optimal_path(ias_scores)
        data["optimal_idx"] = optimal_idx
        data["optimal_IAS"] = optimal_score
        response_list.append(total_response_list[optimal_idx])
    ...
    # assert len(instruction_list) == len(response_list)
    # eval_res = evaluator.evaluate(instruction_list, response_list)
    
    # result = {'data': dataset}
    
    # if eval_model_name == "ft-mistral-7b-instruct-v0.2":
    #     scores = 0.0
    #     valid_cnt = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_model_name", type=str, default="gpt-4o-mini") # Llama-Guard, ft-mistral-7b-instruct-v0.2, gpt-4o-mini
