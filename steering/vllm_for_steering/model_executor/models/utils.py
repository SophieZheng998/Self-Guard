...
from typing import Any, Callable, Literal, Optional, Protocol, Union, overload, List
...
def make_layers(
    ...
    prefix: str,
    # xx
    layer_ids: Optional[dict[int]] = None,
    strength: float = 0.0,
    directions: Optional[dict[int]] = None,
    is_qwen2: bool = False
    ...
    if is_qwen2:
        if layer_ids is not None: # xx
            modules = torch.nn.ModuleList(
                [PPMissingLayer() for _ in range(start_layer)] + [
                    maybe_offload_to_cpu(layer_fn(prefix=f"{prefix}.{idx}", is_steer=bool(idx in layer_ids), layer_idx=idx, strength=strength, directions=directions))
                    for idx in range(start_layer, end_layer)
                ] + [PPMissingLayer() for _ in range(end_layer, num_hidden_layers)])
        else:
            modules = torch.nn.ModuleList(
                [PPMissingLayer() for _ in range(start_layer)] + [
                    maybe_offload_to_cpu(layer_fn(prefix=f"{prefix}.{idx}", is_steer=False, layer_idx=None, strength=strength, directions=directions))
                    for idx in range(start_layer, end_layer)
                ] + [PPMissingLayer() for _ in range(end_layer, num_hidden_layers)])
    else:
        modules = torch.nn.ModuleList(
